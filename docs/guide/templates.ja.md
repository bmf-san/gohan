...
| `archive.html` | `/archives/<year>/` | 年別アーカイブページ |
...
| `paginationPages` | `{{paginationPages .Pagination.CurrentPage .Pagination.TotalPages}}` | ページネーションコントロール用のページ番号スライスを返す（`-1` は省略記号） |
| `pageURL` | `{{pageURL .Pagination.BaseURL 3}}` → `/tags/go/page/3/` | ベースパス内の指定ページ番号の URL を返す。1 ページ目は `baseURL + "/"` |
