...
2. **ブレがある場合**: 以下のいずれかを選択してから実装を進める
   - **Design Doc を修正する**: 実装上より合理的な設計が見つかった場合。修正内容と理由をコメントに残す
   - **Design Doc に従う**: 既存設計に問題がなければそのまま実装する
3. **照合結果**: PR の description に "Design Doc との差分: なし / あり（対応: 〇〇）" を明記する
...
| # | 内容 | 状態 |
|---|---|---|
| - | `Makefile` 追加 (`build`, `test`, `lint`, `serve`, `clean` ターゲット) | ✅ 完了 |
| - | `README.md` 作成（日本語主体・英語併記）— インストール・クイックスタート・設定リファレンス | ✅ 完了 |
| - | ユーザー向けドキュメント作成 (`docs/guide/` 等) — gohan の使い方・設定・テンプレート・タクソノミー | ✅ 完了 |
