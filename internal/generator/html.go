...
func (g *HTMLGenerator) buildJobs(site *model.Site) []writeJob {
	...
	// Virtual pages — generated by SitePlugins (e.g. bookshelf).
	// Each VirtualPage uses a locale-scoped Site copy so that header/footer
	// templates receive the correct Tags, Categories, and ArchiveYears.
	for _, vp := range site.VirtualPages {
		vp := vp
		var base *model.Site
		if b, ok := articleLocaleBases[vp.Locale]; ok {
			base = b
		} else {
			base = localeTaxonomyBase(site, site.Articles)
		}
		d := siteFor(base, nil)
		d.CurrentLocale = vp.Locale
		d.VirtualPageData = vp.Data
		outPath := filepath.Join(g.outDir, vp.OutputPath)
		jobs = append(jobs, writeJob{
			path: outPath,
			tmpl: vp.Template,
			data: d,
...
// TaxonomyURLs returns the canonical URL paths (with trailing slash) for all
// page-1 taxonomy and archive listing pages generated by this site.
// Pagination sub-pages (/page/N/) are intentionally excluded.
// Returned paths are relative to the site root, e.g. "/tags/go/",
// "/categories/architecture/", "/archives/2024/", "/archives/2024/01/".
func TaxonomyURLs(site *model.Site, cfg model.Config) []string {
	...
	localePrefix := func(locale string) string {
	...
	// If i18n is configured iterate per locale; otherwise single pass.
	if len(locales) > 0 {
		for _, locale := range locales {
			pfx := localePrefix(locale)
			...
			for _, tag := range site.Tags {
				...
				if hasArticle {
					urls = append(urls, pfx+"/tags/"+tagNorm(t.Name)+"/")
				}
			}
			...
			for _, cat := range site.Categories {
				...
				if hasArticle {
					urls = append(urls, pfx+"/categories/"+tagNorm(c.Name)+"/")
				}
			}
			...
			for _, a := range site.Articles {
				if a.Locale != locale || a.FrontMatter.Date.IsZero() {
					continue
				}
				y := a.FrontMatter.Date.Year()
				m := int(a.FrontMatter.Date.Month())
				years[y] = true
				months[fmt.Sprintf("%04d/%02d", y, m)] = true
			}
			for y := range years {
				urls = append(urls, fmt.Sprintf("%s/archives/%04d/", pfx, y))
			}
			for ym := range months {
				urls = append(urls, fmt.Sprintf("%s/archives/%s/", pfx, ym))
			}
		}
	} else {
		// No i18n — single locale.
		for _, tag := range site.Tags {
			urls = append(urls, "/tags/"+tagNorm(tag.Name)+"/")
		}
		for _, cat := range site.Categories {
			urls = append(urls, "/categories/"+tagNorm(cat.Name)+"/")
		}
		years := map[int]bool{}
		months := map[string]bool{}
		for _, a := range site.Articles {
		...
		for y := range years {
			urls = append(urls, fmt.Sprintf("/archives/%04d/", y))
		}
		for ym := range months {
			urls = append(urls, fmt.Sprintf("/archives/%s/", ym))
		}
	}
